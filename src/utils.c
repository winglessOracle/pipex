...
/*   Updated: 2023/03/03 16:15:03 by cwesseli      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	init_data(int argc, char **argv, t_data *data)
{
	...
	if (data->fd_in_file < 0 || data->fd_out_file < 0)
	{
		perror("error opening in_file or out_file\n");
		exit(4);
	}
	return (0);
}
...
int	get_processes(char **argv, t_data *data)
{
	...
	if (!data->processes)
		error_msg(5, "Error malloc processes rows\n", data);
	i = 0;
...
void	find_exec_cmd(int i, char **envp, t_data *data)
{
	...
	j = 0;
	cmd_args = ft_split(data->processes[i], ' ');
